...
for i in range(0,len(entries_test)):
    #print(entry)
    files_list.append(entries_test[i])

#print(files_list)
print('length of image file', len(files_list))


# saving the names of the files in a csv file named images_test.csv
with open('images_test.csv', 'w', newline='') as file:
    fieldnames = ['filename']
...
# finding the images_test.csv file
csv_path = ''
for root, _, files in os.walk('.'):
    for name in files:
        # print(files)
        if name == 'images_test.csv':
...
# saving the 2 sets of names and similarity of the files in a csv file named data_test.csv
